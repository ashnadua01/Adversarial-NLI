...
export GLUE_DIR=/kaggle/working/InfoBERT/ANLI/anli_data
export TASK_NAME=${1}
...
max=-1
for file in ${expname}/checkpoint-*; do
  fname=$(basename ${file})
  num=${fname:11}
  if [[ $num -gt $max ]]; then
    max=$num
...
# Now we just run the script normally without multiprocessing
echo "Master port: 1024"  # Random port for single process setup (this is not necessary, just for consistency)
python ./run_anli.py \
